...
-- Track the last focused app for each key
local lastFocusedApp = {}

function launchFocusOrToggle(app, key)
  return function()
    local currentApp = hs.application.frontmostApplication()
    local currentAppName = currentApp and currentApp:name()

    -- Check if the target app is currently frontmost
    if currentAppName and (currentAppName == app or currentAppName:lower():find(app:lower())) then
      -- If this app was the last one we focused with this key, minimize it
      if lastFocusedApp[key] == currentAppName then
        local mainWindow = currentApp:mainWindow()
        if mainWindow then
          mainWindow:minimize()
        end
        lastFocusedApp[key] = nil
      else
        -- Different app is focused, just track it
        lastFocusedApp[key] = currentAppName
      end
    else
      -- App is not frontmost, launch or focus it
      hs.application.launchOrFocus(app)
      lastFocusedApp[key] = app
    end
  end
end
...
function minimizeFrontmostWindow()
  return function()
    local window = hs.window.frontmostWindow()
    if window then
      window:minimize()
...
local bindings = {
  [{ 'alt', 'cmd', 'ctrl', 'shift' }] = {
    -- q = launchOrFocus(''),
    w = launchFocusOrToggle('Zen', 'w'),
    e = launchFocusOrToggle('Finder', 'e'),
    r = launchFocusOrToggle('Ghostty', 'r'),
    s = minimizeFrontmostWindow(),

...
local function is_dark_scheme(scheme_data)
	local bg_color = scheme_data.background
	if not bg_color then return true end

	local r, g, b = hex_to_rgb(bg_color)
	if not r then return true end

	local luminance = get_luminance(r, g, b)
	if not luminance then return true end
...
function M.pick()
	...
	for name, scheme_data in pairs(builtin_schemes) do
		if is_dark_scheme(scheme_data) then
			table.insert(dark_schemes, name)
		end
	end

	table.sort(dark_schemes)

	for _, name in ipairs(dark_schemes) do
		table.insert(choices, {
			id = name,
			label = "🌙 " .. name,
		})
	...
	return wezterm.action.InputSelector {
		title = "🎨 Select Dark Colorschemes",
		choices = choices,
		fuzzy = true,
		fuzzy_description = "🌙 Type to search dark colorschemes...",

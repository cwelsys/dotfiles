...
EVENT_OPTS.validator = optv:new(EVENT_OPTS.schema)
...
-- Additional OS icons
local GLYPH_WINDOWS = nf.md_microsoft_windows --[[ 󰖳 ]]
...
local function create_title(process_name, base_title, max_width, inset)
   ...
   if process_name:len() > 0 then
      -- Get the appropriate OS icon based on platform and process
      local os_icon
      ...
      title = process_name .. ' ' .. os_icon .. ' ' .. base_title
   else
      title = base_title
   end

...
Usage:
    kitten klonopin.py pin              # Pin current tab
    kitten klonopin.py unpin            # Unpin current tab
    ...
    kitten klonopin.py new-tab          # New tab (kitty places it before pinned tabs)
    kitten klonopin.py new-tab --local  # New tab (no cwd)
    kitten klonopin.py move-left        # Move tab left (pinned ordering enforced by kitty)
    kitten klonopin.py move-right       # Move tab right (pinned ordering enforced by kitty)
"""
...
def do_new_tab(boss, local_mode):
    """Open new tab (kitty's _enforce_pinned_ordering places it before pinned tabs)."""
...
def do_move_left(boss):
    """Move current tab left (pinned ordering enforced by TabManager)."""
    tm = boss.active_tab_manager
    if tm is None:
        return
    tm.move_tab(-1)
...
def do_move_right(boss):
    """Move current tab right (pinned ordering enforced by TabManager)."""
    tm = boss.active_tab_manager
    if tm is None:
        return
    tm.move_tab(1)
